...
def notify_signals(signals):
    ...
    for s in buys:
        bd = _breakdown_str(s)
        lines.append(f"🟢 *BUY {s['symbol']}*  score=`{s['score']:+d}`  @{s['price']:.2f}\n   {bd}")

    for s in sells:
        bd = _breakdown_str(s)
        lines.append(f"🔴 *SELL {s['symbol']}*  score=`{s['score']:+d}`  @{s['price']:.2f}\n   {bd}")
...
Mode: {'DRY RUN (No real order submitted)' if is_dry_run else 'LIVE (Real order submitted to broker)'}
    ...
    try:
        ...
        server.starttls()
        server.login(sender_email, sender_password)  # type: ignore[arg-type]
        server.sendmail(sender_email, receiver_email, msg.as_string())  # type: ignore[arg-type]
        server.quit()
